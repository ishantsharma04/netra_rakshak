...
audio_state={"level":0.0,"status":"STARTING","last_alert":0.0,
             "class":"Loading","confidence":0.0,"model":"AST / AudioSet"}
audio_lock=threading.Lock()
AUDIO_THRESHOLD=0.18
AUDIO_COOLDOWN=5.0
AUDIO_SAMPLE_RATE=16000
AUDIO_BUFFER_SECONDS=5
audio_buffer=deque(maxlen=AUDIO_SAMPLE_RATE*AUDIO_BUFFER_SECONDS)
audio_buffer_lock=threading.Lock()
audio_classifier=None
audio_classifier_ready=False
AUDIO_MODEL="MIT/ast-finetuned-audioset-10-10-0.4593"
AUDIO_ALERT_LABELS={
    "Gunshot, gunfire": "Possible Gunshot-like Sound",
    "Explosion": "Possible Explosion-like Sound",
    "Siren": "Siren",
    "Shout": "Shouting",
    "Yell": "Shouting",
}
AUDIO_ALERT_THRESHOLD=0.45
...
def audio_callback(indata, frames, callback_time, status):
    ...
    with audio_lock:
        ...
        elif audio_state["status"] != "CLASSIFYING":
            audio_state["status"]="NORMAL"

def classify_audio_loop():
    global audio_classifier,audio_classifier_ready
    if pipeline is None:
        with audio_lock: audio_state["status"]="MODEL UNAVAILABLE"
        print("Install transformers to enable Stage 2 audio classification.")
        return
    try:
        print("Loading Stage 2 audio model...")
        audio_classifier=pipeline("audio-classification",model=AUDIO_MODEL,device=-1)
        audio_classifier_ready=True
        with audio_lock: audio_state["status"]="LISTENING"
        print("Stage 2 audio model ready.")
    except Exception as e:
        with audio_lock: audio_state["status"]="MODEL ERROR"
        print("Audio model load failed:",e)
        return
    last_run=0.0
    while True:
        time.sleep(1.0)
        if time.time()-last_run < 4.0:
            continue
        with audio_buffer_lock:
            if len(audio_buffer) < AUDIO_SAMPLE_RATE*3:
                continue
            samples=np.asarray(audio_buffer,dtype=np.float32)
        last_run=time.time()
        try:
            with audio_lock: audio_state["status"]="CLASSIFYING"
            preds=audio_classifier({"raw":samples,"sampling_rate":AUDIO_SAMPLE_RATE},top_k=5)
            best=preds[0] if preds else {"label":"Unknown","score":0.0}
            label=best["label"]
            score=float(best["score"])
            display_label=AUDIO_ALERT_LABELS.get(label,label)
            with audio_lock:
                audio_state["class"]=display_label
                audio_state["confidence"]=round(score,3)
                audio_state["status"]="ALERT" if label in AUDIO_ALERT_LABELS and score>=AUDIO_ALERT_THRESHOLD else "LISTENING"
            if label in AUDIO_ALERT_LABELS and score>=AUDIO_ALERT_THRESHOLD:
                now=time.time()
                with audio_lock:
                    last_alert=audio_state.get("last_class_alert",0.0)
                    if now-last_alert>=AUDIO_COOLDOWN:
                        audio_state["last_class_alert"]=now
                        severity="HIGH" if label in ("Gunshot, gunfire","Explosion") else "MEDIUM"
                        add_event("Audio Classification",severity,
                                  f"{display_label} ({score*100:.0f}% confidence)")
        except Exception as e:
            with audio_lock: audio_state["status"]="CLASSIFIER ERROR"
            print("Audio classification error:",e)

def start_audio_monitor():
    try:
        stream=sd.InputStream(channels=1,samplerate=AUDIO_SAMPLE_RATE,
                              blocksize=1024,callback=audio_callback)
        stream.start()
        threading.Thread(target=classify_audio_loop,daemon=True).start()
        return stream
    except Exception as e:
        with audio_lock: audio_state["status"]="UNAVAILABLE"
